			...
			unsigned long event, int enable)
{
	...
	case NOTIFY_EVENT_LANHUB_CONNECT:
		...
		break;
	case NOTIFY_EVENT_POWER_SOURCE:
		pr_info("%s NOTIFY_EVENT_POWER_SOURCE %d\n", __func__, enable);
		if (enable)
...
bool is_snkdfp_usb_device_connected(struct otg_notify *n)
{
	...
	pr_info("%s is_device = %d, power_role = %d\n",
		__func__, u_notify->is_device, u_notify->typec_status.power_role);
	if (u_notify->is_device && u_notify->typec_status.power_role == HNOTIFY_SINK) {
		return true;
	}
